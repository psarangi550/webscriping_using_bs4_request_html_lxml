...
def cmdline_data():
    arguments =argparse.ArgumentParser(description="used for command line")
    #creating the object of Argument Parser
    arguments.add_argument("-u","--url",help="add the url you want to scrap",type=str,default="http://books.toscrape.com/catalogue/a-light-in-the-attic_1000/index.html")
    arguments.add_argument("-f","--file",help="provide a fileName",default="data.json")
    arguments.add_argument("-o","--out",help="provide file output type json/csv",default="json")
    #fetching the value from the arguments
    args=arguments.parse_args()
    return args.url,args.file,args.out

resp=requests.get(cmdline_data()[0],headers=headers)
...
if __name__ == '__main__':
    if cmdline_data()[2]=="json":
        write_to_json(cmdline_data()[1],book_info)
    else:
        write_to_csv(cmdline_data()[1],book_info)
